...
public class TVShowsPage extends BasePage{
	...
	//Locators
	 By btntvshows = By.id("tv.hooq.android:id/btnTvShows");
	 By playButton1 = By.id("tv.hooq.android:id/txtPlay");
	 By MoviesHeaderTitle=By.id("tv.hooq.android:id/headerLogo");
	...
	public boolean fnVerifyTvShowsSection() throws Throwable {
		Reporter.info("HOOQ Android","Verifying TVShows Section");
		Reporter.blnReportTempStatus=true;
		try
		{
			commonPage.fnSelectTab("tvshows");
			if(isElementDisplayed(MoviesHeaderTitle,"TV Shows Page"))
			{
				Reporter.pass("TV Shows Page Screen", "TV Shows Page is displayed");
				...
				if(eleCat.size() >0)
				{
					...
					for(int i=0;i<12;i++)
					{
						...
						arrData=fnGetTVShowsData(arrData, strData);
					}
					if(arrData.size()>0)
					{
						Reporter.pass("TV Shows Section", "TV Shows Section Total Section :- "+ arrData.size() +" is displayed");
						for(int i=0;i<arrData.size();i++)
						{
							String[] arrNewdata=arrData.get(i).split("/");
							Reporter.pass("TV Shows Section", "TV Shows Section :- "+ arrNewdata[0] +" is displayed");
							if(arrNewdata[1].toLowerCase().contains("na"))
							{
								Reporter.fail("TV Shows Section", "TV Shows Section :- "+ arrNewdata[0] +" See All is not displayed");
							}
							else
							{
								Reporter.pass("TV Shows Section", "TV Shows Section :- "+ arrNewdata[0] +" See All is displayed");
							}
							Reporter.pass("TV Shows Section", "TV Shows Section :- "+ arrNewdata[0] +" Total TV Shows :- " + arrNewdata[2] +" is displayed");
						}
					}
					else
					{
						Reporter.fail("TV Shows Section", "TV Shows Section Total Section :- "+ arrData.size() +" is displayed");
					}
					for(int i=0;i<7;i++)
					{
					...
					{
						Reporter.pass("TV Shows Section", "TV Shows Section :- "+ strHeader1 +" Selected and Displayed Inner Header :- "+ strHeader2);
					}
					else
					{
						Reporter.fail("TV Shows Section", "TV Shows Section :- "+ strHeader1 +" Selected and not Displayed Inner Header :- "+ strHeader2);
					}
					String strTotalResult=driver.findElement(innerTotalResult).getText();
					if(strTotalResult.length()>0)
					{
						Reporter.pass("TV Shows Section", "TV Shows Section Total result :- " + strTotalResult +"  is displayed");
					}
					else
					{
						Reporter.fail("TV Shows Section", "TV Shows Section Total result :- " + strTotalResult +"  is not  displayed");
					}
				}
			}
			else
			{
				Reporter.fail("TV Shows Page Screen", "TV Shows Page is not displayed");
			}
	...
	public List<String> fnGetTVShowsData(List<String> arrData,String strData)
	{
		boolean blnAdd=true;
		for(int i=0;i<arrData.size();i++)
		{
	...
	public boolean fnVerifyTvShowsSectionSelectContent() throws Throwable {
		Reporter.info("HOOQ Android","Verifying TVShows Section");
		Reporter.blnReportTempStatus=true;
		try
		{
			...
			if(isElementDisplayed(MoviesHeaderTitle,"TV Shows Page"))
			{
				...
				if(eleListInner.size()>0)
				{
					...
					Reporter.pass("TV Shows Section", "TV Shows Section First TV Shows is Selected successfully.");
				}
				else
				{
					Reporter.fail("TV Shows Section", "TV Shows Section First TV Shows is not Selected");
				}
