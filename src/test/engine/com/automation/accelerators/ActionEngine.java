...
public class ActionEngine extends TestEngine {
                    ...
                    for(int i=1;(!(isElementDisplayedTemp(locator)))|(i<300);i=i+1){
                        ...
                        try{
                            ...
                            if(!(isElementDisplayedTemp(we1))){
                                ...
                                if(objConfig.strOSName.toLowerCase().contains("android")){
	...
	public  boolean doubleTapElement(By by,String locatorName) throws Throwable {
        boolean flag = false;
        try {
			...
			if (!flag) {
				Reporter.fail("DoubleClick", "Unable to doubleclick on "
						+ locatorName);
				return flag;
			} else if (b && flag) {
				Reporter.pass("DoubleClick", "Performed double click action on "
						+ locatorName);
...
//			}
	...
	public  boolean waitForTextOnElementIsPresent(By by,String expectedText ,String locator)
			throws Throwable {
		boolean flag = false;
		WebDriverWait wait = new WebDriverWait(driver, 30);
		try {
			...
			if (!flag) {
				Reporter.fail("waitForTextOnElementIsPresent ",
						" Unabel to find the text "+ expectedText + " on Element "+locator + " is  visible");
			} else if (b && flag) {
				Reporter.pass("waitForTextOnElementIsPresent ",
	...
	public  List<WebElement> getElementsByIosUIAutomation(String locator,String locatorName) throws Throwable {
		boolean flag = false;
	...
	public  boolean isLinkSuccess(String URLName) throws Throwable {
		boolean flag = false;
		...
		try {
			...
			if (!flag) {
				Reporter.fail("IsLinkSuccess ",
						" Failed to veirfy if Link " + URLName
								+ " response code " + respCode);
			} else { 
				Reporter.pass("IsLinkSuccess ",
	...
	public  boolean isLinkSuccessWithOutAuth(String URLName) throws Throwable {
		boolean flag = false;
		System.out.println(URLName);
		int respCode = 0;
		try {
	...
	public  void tapOn(String loginLogo) throws Throwable{
        ...
        MobileElement swt1 = (MobileElement) driver.findElementById("title");
        new TouchAction((MobileDriver) driver).press(swt1).press(swt1).press(swt1).press(swt1).press(swt1).tap(swt1).perform();
        
        
        WebElement el = driver.findElement(By.xpath("//*[@text='Log in'][@resource-id='tv.hooq.android:id/title']"));
        ...
        Reporter.pass("TripleTap", "Succesfully triple tapped on Login title");
    }
	
	public  void seekTo(){
		 /*MobileElement element = (MobileElement) driver.findElement(By.xpath("//[@resource-id='tv.hooq.android:id/seekBar'][@bounds='[0,2303][1440,2399]']"));
        new TouchAction((MobileDriver) driver).moveTo(element).perform();*/
		
		 WebElement el = driver.findElement(By.xpath("//*[@resource-id='tv.hooq.android:id/seekBar']"));
	        Point p = ((Locatable) el).getCoordinates().onPage();
	        ((TouchShortcuts) driver).tap(1,p.getX(),p.getY(),1);
	...
	public void SeekBarTest(){
		WebElement slider=driver.findElement(By.xpath("//*[@resource-id='tv.hooq.android:id/seekBar']"));
	...
	public  void swipeUpOrBottom(boolean bottomToTop) throws Throwable{
		...
		if(bottomToTop){
			...
			//Reporter.pass("SwipeAction", "Swipe from bottom to top has performed");
		} else{
			((AndroidDriver)driver).swipe(startx, endy, startx, starty, 3000);
			//Reporter.pass("SwipeAction", "Swipe from top to bottom has performed");
		}
		 
		 
	}
	public  MobileElement getMobileElement(By b){
		MobileElement element = null;
		try {
			element = (MobileElement) driver.findElement(b);
		} catch (Exception e) {
			Reporter.fnPrintException(e);
		}
		return element;
    ...
    public static String fnGetPWALocators(String strLocatorName)
	{
		String strLocName="";
		Properties prop = new Properties();
		try
		{
			String strDirPath=FileUtilities.fnGetCurrentUserDir();
			String Path=strDirPath+"/src/test/web/com/automation/web/testobjects";
			String strFilePath="";
			if(TestEngine.objConfig.strDriverType.toLowerCase().contains("desktop"))
			{
				strFilePath=Path+"/Desktop.properties";
			}
			else if(TestEngine.objConfig.strDriverType.toLowerCase().contains("mobile"))
			{
				strFilePath=Path+"/Mobile.properties";
			}
			prop.load(new FileInputStream(strFilePath));
			strLocName=prop.getProperty(strLocatorName);
		}
		catch(Exception e){Reporter.fnPrintException(e);}
		return strLocName;
	}
    
    public static void swipe(RemoteWebDriver driver,WebElement elem, Direction right) {
        org.openqa.selenium.Dimension size = elem.getSize();
        int startX = 0;
        int endX = 0;
        int startY = 0;
        int endY = 0;
    ...
    public void scrollElement(AppiumDriver driver,WebElement elem)
    {
    	try
    	{
    		MobileElement topCharts = (MobileElement) elem;
    	    Point point = topCharts.getLocation();
    	    int startY = point.y;
    	    int endY = point.y;
    	    int startX = (int) ((driver.manage().window().getSize().getWidth()) * 0.60);
    	    int endX = (int) ((driver.manage().window().getSize().getWidth()) * 0.40);
    ...
    public void scrollElement(AppiumDriver driver,By loc)
    {
    	try
    	{
    		MobileElement topCharts = (MobileElement) driver.findElement(loc);
    	    Point point = topCharts.getLocation();
    	    int startY = point.y;
    	    int endY = point.y;
    	    int startX = (int) ((driver.manage().window().getSize().getWidth()) * 0.60);
